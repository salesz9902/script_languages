...
def checkrange(c):
    """Returns True if its a special character (not a digit or a character). Else: False"""
    if(ord(c) not in range(97, 122+1) and ord(c) not in range(48, 57+1)):
        return True
    else:
        return False
        ...
        for j, c in enumerate(w):
            if(checkrange(c)):
                if(j > 0 and j < len(w)-1):
                    if(not checkrange(w[j-1]) and not checkrange(w[j+1])):
                        # if(ord(w[j-1]) in range(97, 122+1) and ord(w[j-1]) in range(48, 57+1) and ord(w[j+1]) in range(97, 122+1) and ord(w[j+1]) in range(48, 57+1)):
                        continue
                w = w.replace(c, '')
        t[i] = w
